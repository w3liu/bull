...
func getHeaders(m *codec.Message) {
	...
	m.Target = set(m.Target, "Bull-Service")
...
func setHeaders(m, r *codec.Message) {
	...
	// set headers
	set("Bull-Id", r.Id)
	set("Bull-Service", r.Target)
	set("Bull-Method", r.Method)
	set("Bull-Endpoint", r.Endpoint)
	set("Bull-Error", r.Error)
}

// setupProtocol sets up the old protocol
func setupProtocol(msg *transport.Message) codec.NewCodec {
	service := getHeader("Bull-Service", msg.Header)
	method := getHeader("Bull-Method", msg.Header)
	endpoint := getHeader("Bull-Endpoint", msg.Header)
	protocol := getHeader("Bull-Protocol", msg.Header)
	target := getHeader("Bull-Target", msg.Header)
	topic := getHeader("Bull-Topic", msg.Header)
	...
	if len(method) == 0 {
		msg.Header["Bull-Method"] = endpoint
	}

	// no endpoint then set to method
	if len(endpoint) == 0 {
		msg.Header["Bull-Endpoint"] = method
	}
